					...
					<c:when test="${totalPages > 1}">
						<c:forEach var="i" begin="0" end="${totalPages - 1}">
							<li class="page-item ${currentPage == i ? 'active' : ''}"><a
								class="page-link"
								href="${pageContext.request.contextPath}/user/index?page=${i}&fullname=${param.fullname != null ? param.fullname : ''}&email=${param.email != null ? param.email : ''}&role=${param.role != null ? param.role : ''}">
									${i + 1} </a></li>
						</c:forEach>
					</c:when>
					<c:otherwise>
...
function removeEmptyFields(form) {
    ...
    for (let i = elements.length - 1; i >= 0; i--) {
        ...
        if (element.type === "search" || element.type === "email" || element.tagName === "SELECT") {
            if (!element.value) {
                element.name = ''; 
            }
        }
